...
def collect_genotypes(
    genesymbol: str, factor: str, factors: dict, genotype_table: dict
) -> dict:
    ...
    normalized_genotype = sorted(
        diplotype_to_phenotype_allele_tables(normalize_genotype(diplotype))
    )
...
def construct_factors_dictionary(factors, key):
    ...
    for gene in factors:
        ...
        for factor in factors[gene]:
            result[gene].append({key: factor, "genotypes": factors[gene][factor]})
    return result


def create_phenotype_and_activityscore_table(
    gene_result_diplotype_table: list,
    gene_result_lookup_table: list,
    gene_result_table: list,
) -> Any:
    ...
    for genotype_row in gene_result_diplotype_table:
        ...
        lookup_id_value = indexed_gene_result_lookup[
            genotype_row["functionphenotypeid"]
        ][0]["phenotypeid"]
        ...
        if activityscore:
            activityscores = collect_genotypes(
                genesymbol, activityscore, activityscores, genotype_row
            )
